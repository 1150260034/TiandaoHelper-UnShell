...
const log = {
  ...
  d: function (msg) {
    if (CONFIG.VERBOSE) console.log("[D] " + msg);
  },
  e: function (msg) {
    console.log("[E] " + msg);
  },
...
function hookAllClassLoaders() {
  ...
  Java.perform(function () {
    ...
    try {
      ...
      BaseDexClassLoader.findClass.overload("java.lang.String").implementation =
        function (name) {
...
function hookContextClassLoader() {
  Java.perform(function () {
    ...
    var contextClassLoaderField =
      Thread.class.getDeclaredField("contextClassLoader");
  ...
  Java.perform(function () {
    ...
    try {
      if (DexFile.loadDex) {
        DexFile.loadDex.overload(
          ...
          "java.lang.String",
          "int",
        ).implementation = function (srcName, outName, flags) {
    ...
    try {
      if (DexFile.defineClass) {
        ...
        ).implementation = function (name, loader, buf, offset, len, pd) {
...
function hookNative() {
  ...
  try {
    var android_dlopen_ext_alt = Module.findExportByName(
      "libdl.so",
  ...
  try {
    ...
    exports.forEach(function (exp) {
      if (exp.name === "JNI_OnLoad") {
        log.i(
          "[JNI] JNI_OnLoad found in " + module.name + " at " + exp.address,
        );
...
function tryExtractDex(className, classLoader) {
  ...
  try {
    ...
    if (elements && elements.length > 0) {
      log.d(
        "[Dex] Found " + elements.length + " dex elements for " + className,
      );
...
function tryDumpDexFile(dexFile, tag) {
  try {
    ...
    if (cookie) {
      ...
      var cookieValue = cookie.get(dexFile);
      log.i("[Dex] mCookie: " + cookieValue);
    }

    // 尝试获取 mRawDexFile 或其他DEX数据
    try {
      var rawDexFile = dexFile.getDeclaredField("mRawDexFile");
      if (rawDexFile) {
        rawDexFile.setAccessible(true);
        var rawDex = rawDexFile.get(dexFile);
        if (rawDex) {
          log.i("[Dex] Found mRawDexFile");
        }
      }
    } catch (e) {
      log.d("mRawDexFile not found: " + e);
    }

    // 尝试从pathList中提取DEX
    try {
      var BaseDexClassLoader = Java.use("dalvik.system.BaseDexClassLoader");
      var currentLoader = Java.cast(
      ...
      var pathList = currentLoader.pathList.value;
      if (pathList) {
        var dexElements = pathList.getDeclaredField("dexElements");
        dexElements.setAccessible(true);
        var elements = dexElements.get(pathList);
        ...
        for (var i = 0; i < elements.length; i++) {
          var element = elements[i];
          var dexFileField = element.getDeclaredField("dexFile");
          dexFileField.setAccessible(true);
          var dex = dexFileField.get(element);

          if (dex) {
            // 尝试获取DEX文件路径
            var pathField = element.getDeclaredField("path");
            if (pathField) {
              pathField.setAccessible(true);
              var path = pathField.get(element);
              log.i("[Dex] Found dex at: " + path);
            }
          }
        }
      }
    } catch (e) {
      log.d("Failed to extract dex from pathList: " + e);
    }
  } catch (e) {
